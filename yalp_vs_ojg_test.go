...
// adapted from https://github.com/yalp/jsonpath/blob/master/jsonpath.go
...
func assert(t *testing.T, json interface{}, tests map[string]interface{}) {
	for path, expected := range tests {
		...
		if err != nil {
			...
			println(fmt.Sprintf(">>> Not supported in ojg: %-14s \t(%s)", path, err.Error()))
		} else if !reflect.DeepEqual(actual, expected) {
			t.Errorf("failed: mismatch for %s\nexpected: %+v\nactual: %+v", path, expected, actual)
			println(fmt.Sprintf(">>> Works differntly:     %-14s \tojg: %-50v in  yalp: %-50v", path, ps(actual), ps(expected)))
		} else {
			println(fmt.Sprintf(">>> Works the same way:   %-14s \tojg: %-50v in  yalp: %-50v", path, ps(actual), ps(expected)))
		}
	}
}

func ps(a any) string {
	return fmt.Sprintf("%v", a)
}

func assertError(t *testing.T, json interface{}, tests map[string]string) {
	for path, expectedError := range tests {
		res, err := Read(json, path)
		if err == nil {
			t.Error("path", path, "should fail with", expectedError, "but returned", res)
			println(">>> No Error with ojg:    " + path)
		} else {
			println(">>> Both produce error:   " + path)
		}
	}
}

// hacky way to make the outputs of yalp and ojg comparable
func Read(json any, path string) (any, error) {
	expr, err := jp.ParseString(path)
	if err != nil {
		return nil, err
	}
	results := expr.Get(json)
	if results == nil {
		return []any{}, nil
	}
	if len(results) == 1 && !(strings.Contains(path, "*") || strings.Contains(path, "..")) {
